...
Generates a structured JSON report via LLM, with an HTML-renderable structure.
"""
import os
import datetime
...
Your output MUST be valid JSON matching EXACTLY the schema provided — no markdown, no backticks, just raw JSON."""
...
def _auto_generate_report_data(
    ...
    """Generate a structured report dict using LLM, then enrich with computed stats."""
...
{feature_summary if feature_summary else "  (none)"}

HIGH-CONFIDENCE INDICATORS (≥0.90):
{hc_summary if hc_summary else "  (none)"}
...
"""
    ...
    from .llm import invoke_with_fallback, parse_json_object

    llm_json = None
    text, provider = invoke_with_fallback(
        [SystemMessage(content=REPORT_SYSTEM_PROMPT), HumanMessage(content=prompt_text)],
        temperature=0.2,
        max_tokens=2048,
    )
    if text:
        llm_json = parse_json_object(text)
        if llm_json:
            print(f"[Report] Parsed JSON via {provider}")
        else:
            print(f"[Report] {provider} returned non-JSON; using structured fallback")

    # --- Fallback: generate report from data without LLM ---
    if not llm_json:
        llm_json = _fallback_report(features, page_text)

    return llm_json


def _fallback_report(features: List[Dict], page_text: str) -> Dict:
    """Build a basic report from extracted data without an LLM."""
    by_type = Counter(f.get("feature_type", "other") for f in features)
    ...
    if len(features) > 20 or has_crypto:
        threat_level = "HIGH"
    ...
    key_findings = []
    if has_crypto:
        key_findings.append(f"Cryptocurrency wallets detected ({by_type['crypto_wallet']}), indicating financial transaction activity.")
    if has_email:
        key_findings.append(f"Email addresses identified ({by_type['email']}), potentially linked to threat actor communications.")
    if by_type.get("username", 0):
        key_findings.append(f"Forum usernames/aliases extracted ({by_type['username']}), useful for cross-platform correlation.")
    if by_type.get("ip", 0):
        key_findings.append(f"IP addresses present ({by_type['ip']}), may indicate infrastructure or server hosting.")
    if not key_findings:
        key_findings = ["Document analyzed. Insufficient pattern matches for automated key findings."]

    return {
        "executive_summary": (
            f"Analysis of the target document identified {len(features)} threat indicators across "
            f"{len(by_type)} categories. "
            + ("Cryptocurrency activity and " if has_crypto else "")
            + ("communication identifiers suggest active threat actor operations." if features else "No high-confidence indicators were found.")
        ),
        "threat_level": threat_level,
        "threat_level_rationale": f"Based on {len(features)} extracted indicators across {len(by_type)} categories.",
...
def generate_investigation_report(
    ...
    return {
        ...
        "statistics": {
            ...
            "high_confidence_count": sum(1 for f in features if (f.get("confidence_score") or 0) >= 0.85),
            "avg_confidence": round(
                sum(f.get("confidence_score") or 0.7 for f in features) / max(len(features), 1), 3
            ),
        },
        "indicators": features,
        "analysis": llm_data,
