...
/* ═══════════════════════════════════════════════
   PANEL RESIZE — constants, hook, presets
   ═══════════════════════════════════════════════ */
const AI_PANEL = { default: 420, min: 340, max: 1000 };
const FEATURES_PANEL = { default: 400, min: 320, max: 1000 };
...
/**
 * Drag-to-resize hook for a right-anchored drawer.
 * - dragging LEFT increases width (drawer is anchored to the right edge)
 * - width clamped to [min, max]
 * - persists to localStorage under `storageKey`
 */
const useResizableDrawer = (storageKey, defaultWidth, minWidth, maxWidth) => {
    ...
    return { width, setWidth, resizing, startResize, reset };
};

/** S / M / L / XL preset row that goes in a drawer header. */
const PanelSizePresets = ({ current, onSelect }) => (
    <div className="hidden sm:flex items-center gap-0.5 rounded border border-neutral-200 bg-neutral-100 p-0.5">
        {PANEL_PRESETS.map((p) => {
            const active = Math.abs(current - p.width) < 12;
            return (
                <button
                    key={p.label}
                    onClick={() => onSelect(p.width)}
                    title={`Resize to ${p.width}px`}
...
const Playground = () => {
    ...
    const aiPanel = useResizableDrawer(
        "gudakesa.aiPanel.width",
        AI_PANEL.default,
        AI_PANEL.min,
        AI_PANEL.max
    );
    const featuresPanel = useResizableDrawer(
        "gudakesa.featuresPanel.width",
        FEATURES_PANEL.default,
        FEATURES_PANEL.min,
        FEATURES_PANEL.max
    );
    ...
    const handleDownload = () => {
        ...
        element.download = `${(activePage.page_name || "document").replace(/[^a-z0-9]/gi, "_")}.txt`;
    ...
    const handleSendAiPrompt = async (textToSend) => {
        ...
        try {
            ...
            setAiMessages((prev) => [
                ...
                {
                    sender: "agent",
                    text: "⚠️ AI service unavailable. Please ensure the intelligence service is running on port 8002.",
                },
            ]);
        } finally {
            setAiSending(false);
                ...
                <div className="relative flex flex-1 overflow-hidden bg-neutral-50">
                    ...
                    >
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div
                                                    className={`mt-1.5 flex items-center justify-between font-mono text-[9px] ${
                                                        isSelected ? "text-neutral-300" : "text-neutral-400"
                                                    }`}
                                                >
                        ...
                        <div className="flex items-center justify-between border-t border-neutral-200 bg-neutral-50 p-3 font-mono text-[10px] text-neutral-500">
                            <span>
                                COUNT: <strong className="text-neutral-900">{pages.length}</strong>
                            </span>
                            <span>
                                TOTAL:{" "}
                            ...
                            <>
                                ...
                                <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-2 font-mono text-[11px] text-neutral-500">
                                    <div className="flex items-center gap-6">
                                        <span>
                                            LINES: <strong className="text-neutral-900">{activeDocStats.lines}</strong>
                                        </span>
                                        <span>
                                            WORDS: <strong className="text-neutral-900">{activeDocStats.words}</strong>
                                        </span>
                                        <span>
                                            EXTRACTED FEATURES: <strong className="text-neutral-900">{extractedFeatures.length}</strong>
                                        </span>
                                    </div>
                                    {docSearch && (
                                            ...
                                            <div className="font-mono text-xs leading-relaxed whitespace-pre-wrap break-words text-neutral-800">
                                                {docSearch.trim() !== "" ? (
                                                    activeDocStats.lineArray
                                                        .filter((l) =>
                                                            l.toLowerCase().includes(docSearch.toLowerCase())
                                                        )
                                                        .join("\n") || (
                                                ...
                                                {docSearch.trim() !== "" ? (
                                                    activeDocStats.lineArray
                                                        ...
                                                        .map((line, idx) => (
                                                            <div
                                                                key={idx}
                                                                className="py-0.5 border-b border-neutral-100"
                                                            >
                                                                {line || "\u00A0"}
                                                            </div>
                                                        ))
                    ...
                    {/* ═══════════════════════════════════════════════
                       RIGHT DRAWER 1: EXTRACTED FEATURES (RESIZABLE)
                       ═══════════════════════════════════════════════ */}
                    <aside
                        style={{ "--panel-w": `${featuresPanel.width}px` }}
                        className={`absolute top-0 bottom-0 right-0 z-30 flex flex-col border-l border-neutral-200 bg-white text-neutral-900 transition-[transform,margin-right] duration-300 ease-in-out md:relative w-[var(--panel-w)] ${
                            featuresDrawerOpen
                                ? "translate-x-0"
                                : "translate-x-full md:mr-[calc(var(--panel-w)*-1)]"
                        }`}
                    >
                        {/* Resize handle — features drawer */}
                        {featuresDrawerOpen && (
                            <div
                                onMouseDown={featuresPanel.startResize}
                                onDoubleClick={featuresPanel.reset}
                                title="Drag to resize · Double-click to reset"
                                className="absolute left-0 top-0 bottom-0 z-50 w-1.5 -ml-0.5 cursor-col-resize group"
                            >
                                <div
                                    className={`absolute left-0 top-0 bottom-0 w-0.5 transition-colors ${
                                        featuresPanel.resizing
                                            ? "bg-neutral-900"
                                            : "bg-transparent group-hover:bg-neutral-400"
                                    }`}
                                />
                            </div>
                        )}

                        <div className="flex items-center justify-between border-b border-neutral-200 px-4 py-3 bg-neutral-50 gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                                <span className="text-sm">⚡</span>
                                <h2 className="text-xs font-bold uppercase tracking-wider text-neutral-900 truncate">
                                    Extracted Features
                                </h2>
                                <span className="rounded bg-neutral-900 px-2 py-0.5 font-mono text-[10px] font-bold text-white shrink-0">
                                    {extractedFeatures.length}
                                </span>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0">
                                <PanelSizePresets
                                    current={featuresPanel.width}
                                    onSelect={featuresPanel.setWidth}
                                />
                                <button
                        ...
                        {extractionMessage && (
                            <div className="border-b border-neutral-200 bg-neutral-100 px-4 py-2 text-[11px] font-mono text-neutral-800 flex items-center justify-between gap-2">
                                <span className="truncate">{extractionMessage}</span>
                                {extracting && (
                                    <div className="h-3 w-3 shrink-0 animate-spin rounded-full border-2 border-neutral-900 border-t-transparent" />
                                )}
                            </div>
                        )}
                    ...
                    {/* ═══════════════════════════════════════════════
                       RIGHT DRAWER 2: AI AGENT (RESIZABLE)
                       ═══════════════════════════════════════════════ */}
                    <aside
                        style={{ "--panel-w": `${aiPanel.width}px` }}
                        className={`absolute top-0 bottom-0 right-0 z-30 flex flex-col border-l border-neutral-200 bg-white text-neutral-900 transition-[transform,margin-right] duration-300 ease-in-out md:relative w-[var(--panel-w)] ${
                            aiPanelOpen
                                ? "translate-x-0"
                                : "translate-x-full md:mr-[calc(var(--panel-w)*-1)]"
                    ...
                    >
                        ...
                        {aiPanelOpen && (
                            <div
                                onMouseDown={aiPanel.startResize}
                                onDoubleClick={aiPanel.reset}
                                title="Drag to resize · Double-click to reset"
                                className="absolute left-0 top-0 bottom-0 z-50 w-1.5 -ml-0.5 cursor-col-resize group"
                            >
                                <div
                        ...
                        <div className="flex items-center justify-between border-b border-neutral-200 px-4 py-3 bg-neutral-50 gap-2">
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                                <PanelSizePresets
                                    current={aiPanel.width}
                                    onSelect={aiPanel.setWidth}
                                />
                                <button
                        ...
                        <div className="border-b border-neutral-200 p-3 bg-white flex flex-wrap gap-1.5">
                            <button
                                onClick={() => handleSendAiPrompt("Summarize this document's key findings.")}
                                className="rounded border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[10px] font-semibold text-neutral-700 hover:bg-neutral-100 transition-colors cursor-pointer"
                            >
                                ⚡ Summarize
                            </button>
                            <button
                                onClick={() => handleSendAiPrompt("What usernames and aliases are in the extracted features?")}
                                className="rounded border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[10px] font-semibold text-neutral-700 hover:bg-neutral-100 transition-colors cursor-pointer"
                            >
                                👤 Usernames
                            </button>
                            <button
                                onClick={() => handleSendAiPrompt("What crypto wallets were found? Assess risk.")}
                                className="rounded border border-neutral-200 bg-neutral-50 px-2.5 py-1 text-[10px] font-semibold text-neutral-700 hover:bg-neutral-100 transition-colors cursor-pointer"
                            >
                                💰 Wallets
                            </button>
                            <button
                                onClick={() => handleSendAiPrompt("Generate a bar chart of the extracted features.")}
                                className="rounded border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-[10px] font-semibold text-indigo-700 hover:bg-indigo-100 transition-colors cursor-pointer"
                            >
                                📊 Bar Chart
                            </button>
                            <button
                                onClick={() => handleSendAiPrompt("Show a pie chart of feature distribution.")}
                                className="rounded border border-violet-200 bg-violet-50 px-2.5 py-1 text-[10px] font-semibold text-violet-700 hover:bg-violet-100 transition-colors cursor-pointer"
                            >
                                🥧 Pie Chart
                            </button>
                            <button
                                onClick={() => handleSendAiPrompt("Assess the overall threat level of this document.")}
                                className="rounded border border-rose-200 bg-rose-50 px-2.5 py-1 text-[10px] font-semibold text-rose-700 hover:bg-rose-100 transition-colors cursor-pointer"
                            >
                                🛡️ Threat Level
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 font-sans text-xs bg-neutral-50/50">
                            {aiMessages.map((msg, idx) => (
                                ...
                                >
                                    ...
                                    <div
                                        className={`rounded-lg p-3 text-xs leading-relaxed max-w-[95%] border shadow-xs ${
                                            msg.sender === "user"
                            ...
                            {aiSending && (
                                <div className="flex flex-col items-start">
                                    <span className="font-mono text-[9px] uppercase tracking-wider text-neutral-400 mb-1">
                                        🤖 GUDAKESA AI
                                    </span>
                                    ...
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-mono uppercase tracking-widest text-neutral-400">
                                                GUDAKESA CTI Platform — Investigation Report
                                            </p>
                                            <h1 className="text-xl font-bold mt-0.5">
                                                {reportData.metadata?.page_name || "Dark Web Investigation"}
                                            </h1>
                                        </div>
                                    </div>
                                ...
                                <div className="flex flex-col items-end gap-2">
                                    ...
                                    <span
                                        className={`mt-2 px-3 py-1 rounded-full text-xs font-bold ${
                                            reportData.analysis?.threat_level === "CRITICAL"
                                                ? "bg-red-600 text-white"
                                                : reportData.analysis?.threat_level === "HIGH"
                                                ? "bg-orange-500 text-white"
                                                : reportData.analysis?.threat_level === "MEDIUM"
                                                ? "bg-yellow-400 text-neutral-900"
                                                : reportData.analysis?.threat_level === "LOW"
                                                ? "bg-green-500 text-white"
                                                : "bg-neutral-600 text-white"
                                        }`}
                                    >
                                        {reportData.analysis?.threat_level || "UNKNOWN"}
                                    </span>
                                </div>
                            </div>
                            <div className="mt-4 border-t border-neutral-700 pt-3">
                                <p className="text-[10px] font-mono text-neutral-500">
                                    {reportData.metadata?.classification}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="p-8 space-y-8 bg-neutral-50">
                                ...
                                {[
                                    {
                                        ...
                                        value: reportData.metadata?.total_indicators,
                                        icon: "📍",
                                    },
                                    {
                                        label: "High Confidence",
                                        value: reportData.statistics?.high_confidence_count,
                                        icon: "✅",
                                    },
                                    {
                                        label: "Avg Confidence",
                                        value: `${((reportData.statistics?.avg_confidence || 0) * 100).toFixed(0)}%`,
                                        icon: "📊",
                                    },
                                ].map((s) => (
                                    <div
                                        key={s.label}
                                        className="bg-white border border-neutral-200 rounded-xl p-4 text-center shadow-sm"
                                    >
                            ...
                            <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm">
                                ...
                                {reportData.analysis?.threat_level_rationale && (
                                    <p className="mt-3 text-xs text-neutral-500 italic border-t border-neutral-100 pt-3">
                                        {reportData.analysis.threat_level_rationale}
                                    </p>
                                )}
                            </div>
                                ...
                                <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm">
                                    ...
                                    <ul className="space-y-2">
                                        {reportData.analysis.key_findings.map((f, i) => (
                                            ...
                                            >
                                                <span className="mt-0.5 h-5 w-5 flex-shrink-0 rounded-full bg-neutral-900 text-white text-[10px] flex items-center justify-center font-bold">
                                                    {i + 1}
                                                </span>
                                    ...
                                    <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm">
                                        ...
                                        <dl className="space-y-3">
                                            {[
                                                [
                                                    "Motivation",
                                                    reportData.analysis.threat_actor_profile.likely_motivation,
                                                ],
                                                [
                                                    "OPSEC",
                                                    reportData.analysis.threat_actor_profile.operational_security,
                                                ],
                                                [
                                                    "Sophistication",
                                                    reportData.analysis.threat_actor_profile.estimated_sophistication,
                                                ],
                                            ].map(
                                                ([k, v]) =>
                                                    v && (
                                                        <div key={k}>
                                                            <dt className="text-[10px] font-mono uppercase text-neutral-400">
                                                                {k}
                                                            </dt>
                                                            <dd className="text-xs text-neutral-800 mt-0.5">
                                                                {v}
                                                            </dd>
                                                        </div>
                                                    )
                                            ...
                                            {reportData.analysis.threat_actor_profile
                                                .indicators_of_attribution?.length > 0 && (
                                                <div>
                                                    <dt className="text-[10px] font-mono uppercase text-neutral-400">
                                                        Attribution Indicators
                                                    </dt>
                                                    <dd className="mt-1 flex flex-wrap gap-1">
                                                        {reportData.analysis.threat_actor_profile.indicators_of_attribution.map(
                                                            (a, i) => (
                                                                <span
                                                                    key={i}
                                                                    className="font-mono text-[9px] bg-neutral-100 border border-neutral-200 rounded px-1.5 py-0.5"
                                                                >
                                                                    {a}
                                                                </span>
                                                            )
                                ...
                                {reportData.analysis?.ioc_analysis && (
                                    <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm">
                                        ...
                                        <div className="space-y-1 mb-3">
                                            {(reportData.analysis.ioc_analysis.high_value || [])
                                                .slice(0, 8)
                                                .map((ioc, i) => (
                                                    <div
                                                        key={i}
                                                        className="font-mono text-[10px] bg-neutral-50 border border-neutral-200 rounded px-2 py-1 text-neutral-800 break-all"
                                                    >
                                                        {ioc}
                                                    </div>
                                                ))}
                                ...
                                <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden shadow-sm">
                                    <div className="px-6 py-4 border-b border-neutral-100">
                                        <h2 className="text-xs font-bold uppercase tracking-wider text-neutral-500">
                                            Extracted Indicators ({reportData.indicators.length})
                                        </h2>
                                    </div>
                                            ...
                                            <thead>
                                                <tr className="bg-neutral-50 border-b border-neutral-100">
                                                    {["Type", "Value", "Confidence", "Context"].map((h) => (
                                                        <th
                                                            key={h}
                                                            className="px-4 py-2 text-left font-mono text-[10px] uppercase text-neutral-400"
                                                        >
                                                            {h}
                                                        </th>
                                                    ))}
                                                ...
                                                {reportData.indicators.slice(0, 30).map((ind, i) => (
                                                    ...
                                                    >
                                                        <td className="px-4 py-2">
                                                            <span className="rounded-full border border-neutral-200 bg-neutral-100 px-2 py-0.5 font-mono text-[9px] text-neutral-700">
                                                                {ind.feature_type}
                                                            </span>
                                                        </td>
                                                        <td className="px-4 py-2 font-mono text-[10px] text-neutral-900 max-w-[200px] truncate">
                                                            {ind.feature_value}
                                                        </td>
                                                        <td className="px-4 py-2">
                                                            <div className="flex items-center gap-1.5">
                                                                <div className="h-1.5 w-16 rounded-full bg-neutral-100 overflow-hidden">
                                                                    <div
                                                                        ...
                                                                        style={{
                                                                            width: `${(ind.confidence_score || 0) * 100}%`,
                                                                        }}
                                                                    />
                                                                </div>
                                                                <span className="font-mono text-[9px] text-neutral-500">
                                                                    {((ind.confidence_score || 0) * 100).toFixed(0)}%
                                                                </span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-2 text-neutral-500 max-w-[250px] truncate text-[10px]">
                                                            {ind.context}
                                                        </td>
                                                    </tr>
                                                ))}
